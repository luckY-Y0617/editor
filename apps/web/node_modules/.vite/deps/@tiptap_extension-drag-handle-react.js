...
import {
  ...
  getUpdatedPosition,
  isFirefox
} from "./chunk-G27H5B5Q.js";
import "./chunk-5X2U4L4S.js";
import {
  Decoration,
  DecorationSet
} from "./chunk-GG22JV5X.js";
import {
  AllSelection,
  Fragment,
  NodeRange,
...
function getNodeRangeDecorations(ranges) {
  ...
  const decorations = [];
  const doc2 = ranges[0].$from.node(0);
...
function getSelectionRanges($from, $to, depth, options = {}) {
  ...
  const offset = nodeRange.depth === 0 ? 0 : doc2.resolve(nodeRange.start).posAtIndex(0);
  nodeRange.parent.forEach((node, pos) => {
    const from3 = offset + pos;
    const to = from3 + node.nodeSize;
    const contentBounds = getNodeContentBounds(from3, node.nodeSize, node);
    ...
    if (from3 < nodeRange.start || from3 >= nodeRange.end) {
      return;
    }
    if (!overlapsNodeContent) {
      return;
    }
    const selectionRange = new SelectionRange(doc2.resolve(from3), doc2.resolve(to));
...
var NodeRangeBookmark = class _NodeRangeBookmark {
  ...
  resolve(doc2) {
    const $anchor = doc2.resolve(this.anchor);
    const $head = doc2.resolve(this.head);
...
var NodeRangeSelection = class _NodeRangeSelection extends Selection {
  constructor($anchor, $head, depth, bias = 1) {
    const { doc: doc2 } = $anchor;
    const isCursor = $anchor === $head;
    const isCursorAtEnd = $anchor.pos === doc2.content.size && $head.pos === doc2.content.size;
    const $correctedHead = isCursor && !isCursorAtEnd ? doc2.resolve($head.pos + (bias > 0 ? 1 : -1)) : $head;
    const $correctedAnchor = isCursor && isCursorAtEnd ? doc2.resolve($anchor.pos - (bias > 0 ? 1 : -1)) : $anchor;
  ...
  map(doc2, mapping) {
    const $anchor = doc2.resolve(mapping.map(this.anchor));
    const $head = doc2.resolve(mapping.map(this.head));
  ...
  extendBackwards() {
    ...
    const $from = doc2.resolve(Math.max(0, firstRange.$from.pos - 1));
  ...
  extendForwards() {
    ...
    const $to = doc2.resolve(Math.min(doc2.content.size, lastRange.$to.pos + 1));
    return new _NodeRangeSelection(this.$anchor, $to, this.depth);
  }
  static fromJSON(doc2, json) {
    return new _NodeRangeSelection(doc2.resolve(json.anchor), doc2.resolve(json.head));
  }
  static create(doc2, anchor, head, depth, bias = 1) {
    return new this(doc2.resolve(anchor), doc2.resolve(head), depth, bias);
  }
  getBookmark() {
  ...
  addKeyboardShortcuts() {
    return {
      ...
      "Shift-ArrowUp": ({ editor }) => {
        ...
        const { doc: doc2, selection, tr } = state;
        const { anchor, head } = selection;
        if (!isNodeRangeSelection(selection)) {
          const nodeRangeSelection2 = NodeRangeSelection.create(doc2, anchor, head, depth, -1);
      ...
      "Shift-ArrowDown": ({ editor }) => {
        ...
        const { anchor, head } = selection;
        if (!isNodeRangeSelection(selection)) {
          const nodeRangeSelection2 = NodeRangeSelection.create(doc2, anchor, head, depth);
          tr.setSelection(nodeRangeSelection2);
          view.dispatch(tr);
          return true;
        }
      ...
      "Mod-a": ({ editor }) => {
        ...
        const nodeRangeSelection = NodeRangeSelection.create(doc2, 0, doc2.content.size, depth);
        tr.setSelection(nodeRangeSelection);
        view.dispatch(tr);
        return true;
      }
        ...
        props: {
          ...
          handleDOMEvents: {
            mousedown: (view, event) => {
              ...
              const isMac2 = /Mac/.test(navigator.platform);
              const isShift = !!event.shiftKey;
              const isControl = !!event.ctrlKey;
              const isAlt = !!event.altKey;
              const isMeta = !!event.metaKey;
              const isMod = isMac2 ? isMeta : isControl;
              ...
              document.addEventListener(
                ...
                () => {
                  ...
                  const nodeRangeSelection = NodeRangeSelection.create(doc2, $anchor.pos, $head.pos, this.options.depth);
                  tr.setSelection(nodeRangeSelection);
                  view.dispatch(tr);
                },
                { once: true }
...
function isNearEdge(coords, element2, config) {
...
function calculateEdgeDeduction(coords, element2, config, depth) {
  if (!element2 || config.edges.length === 0) {
    return 0;
  }
  if (isNearEdge(coords, element2, config)) {
...
function findBestDragTarget(view, coords, options) {
  ...
  const $pos = doc2.resolve(posInfo.pos);
  const rules = [];
...
function findClosestTopLevelBlock(element2, view) {
...
function getDragHandleRanges(event, editor, nestedOptions, dragContext) {
  ...
  if ((nestedOptions == null ? void 0 : nestedOptions.enabled) && (dragContext == null ? void 0 : dragContext.node) && dragContext.pos >= 0) {
    ...
    return [
      {
        $from: doc2.resolve(nodeStart),
        $to: doc2.resolve(nodeEnd)
      }
    ];
  }
  ...
  const $to = doc2.resolve(result.pos + result.resultNode.nodeSize + offset);
...
function dragHandler(event, editor, nestedOptions, dragContext) {
  ...
  const direction = getDraggedBlockDir(view, from3);
  ...
  let selection;
  if (isNestedDrag) {
    slice = view.state.doc.slice(from3, to);
    selection = NodeSelection.create(view.state.doc, from3);
  } else {
    selection = NodeRangeSelection.create(view.state.doc, from3, to);
    slice = selection.content();
  }
  ranges.forEach((range) => {
    const element2 = getDraggedBlockElement(view, range.$from.pos);
    if (!element2) {
      return;
    }
...
var DragHandlePlugin = ({
  ...
  function repositionDragHandle(dom) {
    ...
    computePosition(virtualElement, element2, computePositionConfig).then((val) => {
