...
function rowIsHeader(map, table, row) {
  ...
  for (let col = 0; col < map.width; col++)
    if (((_table$nodeAt = table.nodeAt(map.map[col + row * map.width])) === null || _table$nodeAt === void 0 ? void 0 : _table$nodeAt.type) != headerCell)
      return false;
  return true;
}
...
function splitCellWithType(getCellType) {
  return (state, dispatch) => {
    ...
    if (dispatch) {
      ...
      if (sel instanceof CellSelection)
        tr.setSelection(new CellSelection(tr.doc.resolve(sel.$anchorCell.pos), lastCell ? tr.doc.resolve(lastCell) : void 0));
      dispatch(tr);
    }
    return true;
  };
...
function handleDecorations(state, cell) {
  ...
  for (let row = 0; row < map.height; row++) {
    ...
    if ((col == map.width - 1 || map.map[index] != map.map[index + 1]) && (row == 0 || map.map[index] != map.map[index - map.width])) {
      ...
      if ((_columnResizingPlugin = columnResizingPluginKey.getState(state)) === null || _columnResizingPlugin === void 0 ? void 0 : _columnResizingPlugin.dragging)
...
//# sourceMappingURL=chunk-66SEZEPO.js.map
