...
public sealed class HostDataSeedContributor : IDataSeedContributor, ITransientDependency
{
    private const string SeedMarker = "[Seed] HostDataSeed";
    private const string DefaultAdminPassword = "1q2w3E*";

    private static readonly IReadOnlyList<string> HostAdminPermissionDefaults = ["*"];
    ...
    public async Task SeedAsync(DataSeedContext context)
    {
        ...
        using (_currentTenant.Change(null))
        {
            ...
            var role = await UpsertRoleAsync(
                ...
                orderNum: 0,
                isEnabled: true,
                description: $"{SeedMarker} Host role");

            await GrantRolePermissionsAsync(role.Id, HostAdminPermissionDefaults);
